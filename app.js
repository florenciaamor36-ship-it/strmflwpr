    ...
    async function verificarSuscripcion(uid) {
      ...
      try {
        ...
        if(!snap.exists) {
          // Si no existe el doc, verificamos la fecha de creación del usuario
          const creationTime = currentUser && currentUser.metadata && currentUser.metadata.creationTime
            ? new Date(currentUser.metadata.creationTime)
            : ahora;

          const fechaFinPrueba = new Date(creationTime);
          fechaFinPrueba.setDate(fechaFinPrueba.getDate() + DIAS_PRUEBA_GRATIS);

          const trialData = {
            fechaPrimerIngreso: creationTime.toISOString(),
            estado: "prueba",
            fechaExpiracion: fechaFinPrueba.toISOString()
          };

          await suscRef.set(trialData);

          if(ahora < fechaFinPrueba) {
    ...
    function aplicarBloqueoApp() {
      const currentTab = document.querySelector(".nav-btn.active")?.dataset.nav;
      const bloqueoMsg = document.getElementById("bloqueoAppMsg");

      if(planEstado === "bloqueado" && currentTab !== "ajustes" && currentTab !== "micuenta") {
        if(!bloqueoMsg) {
          ...
          msg.innerHTML = `
            <div class="bloqueo-content">
              <div style="width:80px; height:80px; background:rgba(241,39,17,0.2); border-radius:50%; display:flex; align-items:center; justify-content:center; margin:0 auto 1.5rem auto; border:2px solid #f12711;">
                <i class="fas fa-lock" style="font-size:2.5rem; color:#f12711;"></i>
              </div>
              <h2 style="margin:0 0 1rem 0; font-size:1.8rem; font-weight:800; background:linear-gradient(135deg,#fff,#f12711); -webkit-background-clip:text; background-clip:text; color:transparent;">Acceso Bloqueado</h2>
              <p style="margin:0 0 2rem 0; font-size:1rem; line-height:1.6; opacity:0.9;">Tu suscripción ha expirado. Para seguir gestionando tus perfiles y ventas, por favor activa un nuevo token.</p>
              <button onclick="cambiarTab('ajustes')" class="btn btn-primary" style="width:100%; padding:1rem; font-size:1rem; font-weight:800; text-transform:uppercase; letter-spacing:1px;">
                <i class="fas fa-key"></i> Ir a Ajustes / Activar Token
              </button>
              <p style="margin-top:1.5rem; font-size:0.8rem; opacity:0.7;"><i class="fas fa-info-circle"></i> Tus datos permanecen guardados y seguros.</p>
            </div>
          `;
          const appContainer = document.getElementById("appContainer");
          if(appContainer) appContainer.appendChild(msg);
        }
      } else {
    ...
    async function venderPerfil(id, datos) {
      if(planEstado === "bloqueado") { toast("❌ Acceso bloqueado: Solo lectura. Ingresá un token en Ajustes."); return; }
    ...
    async function renovarPerfil(id) {
      if(planEstado === "bloqueado") { toast("❌ Acceso bloqueado: Solo lectura."); return; }
      const p = perfiles.find(p => p.id === id);
      if(p) {
