...
class FirestoreService {
  ...
  Stream<List<PlatformModel>> platformsStream(String userId) {
    return _db
        .collection(AppConstants.platformsCollection)
  ...
  Stream<List<AccountModel>> accountsByPlatformStream(
      String userId, String platformId) {
    return _db
        .collection(AppConstants.accountsCollection)
        .where('userId', isEqualTo: userId)
  ...
  Stream<List<ProfileModel>> profilesByAccountStream(
  ...
  Stream<List<ProfileModel>> availableProfilesStream(
      String userId, String platformId) {
    return _db
        .collection(AppConstants.profilesCollection)
        .where('userId', isEqualTo: userId)
  ...
  /// Real-time dashboard stats stream
  ...
  Future<String> addSale(SaleModel sale) async {
    ...
    if (sale.profileId.isNotEmpty) {
      ...
      batch.update(profileRef, {
        ...
        'currentClientName': sale.clientName,
      });
    }
  ...
  Future<void> renewSale(
      String saleId, String profileId, DateTime newExpiration, double price,
      {String notes = ''}) async {
    final saleDoc = await _db
        .collection(AppConstants.salesCollection)
        .doc(saleId)
        .get();
    ...
    batch.update(saleRef, {
      ...
      'status': 'active',
      'renewalCount': sale.renewalCount + 1,
    ...
    final renewalRef =
        _db.collection(AppConstants.renewalsCollection).doc();
    final renewal = RenewalModel(
      id: renewalRef.id,
      saleId: saleId,
      userId: sale.userId,
      ...
      previousExpiration: sale.expirationDate,
  ...
  Future<void> markReminderSent(String saleId, int daysBefore) async {
    await _db.collection(AppConstants.salesCollection).doc(saleId).update({
      'remindersSent': FieldValue.arrayUnion([daysBefore]),
    });
  }
  ...
  Stream<List<TemplateModel>> templatesStream(String userId) {
    return _db
        ...
        .snapshots()
        .map((snap) =>
            snap.docs.map((d) => TemplateModel.fromFirestore(d)).toList());
  }
  ...
  Stream<List<RenewalModel>> renewalsForSaleStream(String saleId) {
    return _db
        .collection(AppConstants.renewalsCollection)
        .where('saleId', isEqualTo: saleId)
        .orderBy('renewedAt', descending: true)
        .snapshots()
        .map((snap) =>
            snap.docs.map((d) => RenewalModel.fromFirestore(d)).toList());
  }
  ...
  Stream<Map<String, Map<String, int>>> inventoryStream(String userId) {
    return _db
      ...
      for (final profile in profiles) {
        ...
        result.putIfAbsent(
            platform, () => {'total': 0, 'available': 0, 'sold': 0});
  ...
  /// Fetch a sale by client token (for client page)
  Future<SaleModel?> getSaleByToken(String token) async {
    final snap = await _db
        .collection(AppConstants.salesCollection)
        .where('clientToken', isEqualTo: token)
        .limit(1)
        .get();
